...
export const getAvailableDrivers = (): Staff[] =>
  mockStaff.filter((s) => s.role === "driver" && s.isOnDuty && !s.currentTaskId);
...
export const getOrderNotificationsByRoom = (roomNumber: string): MealOrderNotification[] =>
...
export const getCelebrationTasksByRoom = (roomNumber: string): CelebrationTask[] =>
...
export const createInspectionTask = (cleaningTask: Task, inspectorId: string): Task => {
  const now = new Date();
