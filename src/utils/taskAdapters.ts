...
function shuttleStatusToUnifiedStatus(
...
// ヘルプステータスから統合ステータスへ
export function helpStatusToUnifiedStatus(
...
// スタッフIDでフィルタリング
export function filterTasksByStaff(
	tasks: UnifiedTask[],
	staffId: string,
): UnifiedTask[] {
...
export function getDetailedStatus(task: UnifiedTask): string {
	switch (task.type) {
		...
		case "shuttle":
			return task.shuttle
				? getShuttleStatusLabel(task.shuttle.shuttleStatus)
				: task.status;
		case "celebration":
			return task.status === "in_progress" ? "対応中" : task.status;
		case "help_request":
			return task.helpRequest
				? getHelpStatusLabel(task.helpRequest.helpStatus)
				: task.status;
	}
}
