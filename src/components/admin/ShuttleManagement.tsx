...
const ShuttleTaskCard = ({
	...
	const driver = task.assignedDriverId
		? getStaffById(task.assignedDriverId)
		: null;
...
const VehicleCard = ({
	...
	const driver = vehicle.currentDriverId
		? getStaffById(vehicle.currentDriverId)
		: null;
	...
	return (
		...
		>
			...
			{vehicle.notes && (
				<div className="mt-1 text-xs text-[var(--shu)]">{vehicle.notes}</div>
			)}
		</button>
	);
};
			...
			<div className="relative bg-white rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
				...
				<div className="p-4">
					...
					<div className="shoji-panel p-3 mb-4">
						<div className="text-sm text-[var(--nezumi)] mb-1">
							{task.scheduledTime}
						</div>
						<div className="font-display font-medium text-[var(--sumi)]">
					...
					{/* Vehicle selection */}
					<div className="mb-4">
						<label className="block text-sm font-medium text-[var(--sumi)] mb-2">
							車両
						</label>
				...
				<div className="p-4 space-y-4">
					...
					<div className="shoji-panel p-4">
						...
						<div className="flex items-center gap-3">
							<div className="text-center">
								...
								<div className="font-medium text-[var(--sumi)]">
									{task.pickupLocation}
								</div>
							</div>
						...
						<div className="mt-2 text-sm text-[var(--nezumi)]">
							予定時刻: {task.scheduledTime} / 所要時間: 約
					...
					<div className="shoji-panel p-4">
						...
						<div className="space-y-2">
							...
							<div className="flex items-center gap-2">
								<span className="text-sm text-[var(--nezumi)] w-16">
									ドライバー:
								</span>
								{driver ? (
									<span className="font-medium text-[var(--sumi)]">
...
export const ShuttleManagement = () => {
	const [shuttleTasks, setShuttleTasks] =
		useState<ShuttleTask[]>(mockShuttleTasks);
	...
	const stats = useMemo(() => {
		...
		const completed = shuttleTasks.filter(
			(t) => t.shuttleStatus === "completed",
		).length;
	...
	const filteredTasks = useMemo(() => {
		...
		return tasks.sort((a, b) => {
			if (a.shuttleStatus === "completed" && b.shuttleStatus !== "completed")
				return 1;
			if (a.shuttleStatus !== "completed" && b.shuttleStatus === "completed")
				return -1;
	...
	const selectedTask = selectedTaskId
		? shuttleTasks.find((t) => t.id === selectedTaskId)
		: null;
