...
import { TimelineIcon, RoomIcon, CelebrationIcon, CheckIcon, ChevronRightIcon } from "../ui/Icons";
...
const CurrentTimeIndicator = ({ slots }: { slots: string[] }) => {
  ...
  const position =
    roomWidth +
    ((currentTimeInMinutes - startTime) / (endTime - startTime)) * (totalSlots * slotWidth);
  ...
  return (
    <div
      ...
      className={`absolute top-1 bottom-1 rounded text-white text-xs flex items-center px-2 overflow-hidden transition-all hover:brightness-110 ${categoryColors[task.category]} ${statusStyles[task.status]} ${
        task.status !== "completed" ? "cursor-grab active:cursor-grabbing" : "cursor-default"
...
const RoomRow = ({
  ...
  const getTimeFromPosition = (clientX: number, containerRect: DOMRect): string => {
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <span className="font-display font-medium text-[var(--sumi)]">{getRoomName(roomId)}</span>
          {reservation?.anniversary && (
            <CelebrationIcon size={14} className="text-[var(--kincha)]" />
          )}
        </div>
        {reservation && (
          <p className="text-xs text-[var(--nezumi)] truncate mt-0.5">{reservation.guestName}</p>
        )}
      </div>
...
const RoomDetail = ({ roomId, reservation, tasks }: RoomDetailProps) => {
    ...
    <div className="shoji-panel animate-slide-in-right">
      <div className="p-4 border-b border-[rgba(45,41,38,0.08)] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-display font-medium text-[var(--sumi)]">{getRoomName(roomId)}</h3>
          {reservation?.anniversary && (
            <span className="badge badge-anniversary">
              <CelebrationIcon size={12} />
              {reservation.anniversary.type === "birthday" ? "誕生日" : "記念日"}
            </span>
          )}
        </div>
      </div>
        ...
        <div className="p-4 border-b border-[rgba(45,41,38,0.06)] bg-[var(--shironeri-warm)]">
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">{ROOM_TYPE_LABELS[reservation.roomType]}</p>
            </div>
            <div>
            ...
            <div className="mt-3 pt-3 border-t border-[rgba(45,41,38,0.08)]">
              ...
              <div className="flex flex-wrap gap-1">
                {reservation.specialRequests.map((req, idx) => (
                  <span key={idx} className="text-xs px-2 py-0.5 bg-white rounded">
                    {req}
                  </span>
                ))}
              </div>
      ...
      <div className="divide-y divide-[rgba(45,41,38,0.04)]">
        ...
        {sortedTasks.length === 0 && (
          <div className="p-8 text-center">
            <TimelineIcon size={32} className="mx-auto text-[var(--nezumi-light)] mb-2" />
...
export const RoomTimeline = () => {
  ...
  // Get unique room IDs from reservations
  const rooms = useMemo(() => {
    const roomSet = new Set(mockReservations.map((r) => r.roomId));
    return Array.from(roomSet).sort();
  }, []);

  const getReservationByRoom = (roomId: string): Reservation | null => {
    return mockReservations.find((r) => r.roomId === roomId) || null;
  };

  const getTasksByRoom = (roomId: string): Task[] => {
    return mockTasks.filter((t) => t.roomId === roomId);
  };

  const selectedReservation = selectedRoom ? getReservationByRoom(selectedRoom) : null;
  ...
  const handleDrop = useCallback(
    (taskId: string, newTime: string, newRoom: string) => {
      if (dragInfo) {
        console.log(
          `タスク ${taskId} を ${dragInfo.originalTime} から ${newTime} に、${getRoomName(dragInfo.originalRoom)} から ${getRoomName(newRoom)} に移動`,
        );
  ...
  const inProgressTasksCount = mockTasks.filter((t) => t.status === "in_progress").length;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="shoji-panel p-4 border-l-3 border-l-[var(--ai)]">
          <p className="text-2xl font-display font-semibold text-[var(--sumi)]">{rooms.length}</p>
      ...
      <div className="shoji-panel p-4">
        <div className="flex flex-wrap items-center gap-4 text-sm">
          ...
          <span className="border-l border-[rgba(45,41,38,0.2)] pl-4 text-[var(--nezumi)]">
            <ChevronRightIcon size={14} className="inline -rotate-90 text-[var(--shu)]" /> 現在時刻
          </span>
        </div>
      </div>
            ...
            <div className="relative" onDragEnd={handleDragEnd}>
              ...
              {rooms.map((roomId) => (
                <RoomRow
                  key={roomId}
                  roomId={roomId}
                  reservation={getReservationByRoom(roomId)}
                  tasks={getTasksByRoom(roomId)}
                  slots={timeSlots}
                  onRoomClick={() => setSelectedRoom(selectedRoom === roomId ? null : roomId)}
                  isSelected={selectedRoom === roomId}
                  onDragStart={handleDragStart}
                  onDrop={handleDrop}
                  draggingTaskId={dragInfo?.taskId || null}
                  isDragOver={dragOverRoom === roomId}
                  onDragEnter={() => setDragOverRoom(roomId)}
