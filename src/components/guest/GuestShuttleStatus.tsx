...
import type { ShuttleTask, ShuttleStatus, ShuttleMessage, ShuttleMessageType } from "../../types";
    ...
    return (
      <div className="bg-[var(--aotake)]/10 border-2 border-[var(--aotake)] rounded-2xl p-6 text-center">
        <div className="flex items-center justify-center gap-3 mb-2">
          ...
          <span className="text-xl font-bold text-[var(--aotake)]">到着をお知らせしました</span>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="text-left">
        <div>今、到着しました</div>
        <div className="text-sm font-normal opacity-80">タップしてドライバーにお知らせ</div>
      </div>
    </button>
  );
};
  ...
  <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        {subValue && <div className="text-sm text-gray-600 mt-0.5">{subValue}</div>}
      </div>
    </div>
  </div>
);
...
export const GuestShuttleStatus = ({ taskId }: GuestShuttleStatusProps) => {
  ...
  const [messages, setMessages] = useState<ShuttleMessage[]>([]);

  // ゲストID（実際は認証から取得）
  const guestId = "guest-" + (taskId || "default");
  const guestName = task?.guestName || "ゲスト";
  ...
  useEffect(() => {
    const loadTask = () => {
      ...
      const shuttleTask = taskId ? getShuttleTaskById(taskId) : getShuttleTaskById("SHT001"); // デモ用デフォルト
  ...
  // メッセージ送信
  const handleSendMessage = (content: string, messageType: ShuttleMessageType) => {
    if (!task) return;

    const newMessage: ShuttleMessage = {
      id: `msg-${Date.now()}`,
      shuttleTaskId: task.id,
      senderType: "guest",
      senderId: guestId,
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl p-8 shadow-lg text-center max-w-md">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">送迎情報が見つかりません</h2>
  ...
  const canNotify = task.shuttleStatus === "heading" || task.shuttleStatus === "arrived";
    ...
    >
      ...
      <main className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-lg text-center">
          ...
          {isCompleted && <p className="text-gray-600">ご利用ありがとうございました</p>}
        </div>
        ...
        {!isCompleted && (
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[var(--ai)]/10 flex items-center justify-center">
                  <MessageIcon size={20} className="text-[var(--ai)]" />
                </div>
                <div>
                  <p className="font-bold text-gray-900">ドライバーに連絡</p>
            ...
            {/* Messages content */}
            {showMessages && (
              <div className="px-4 pb-4 space-y-4 animate-fade-in border-t border-gray-100 pt-4">
                {/* Quick messages for guest */}
                <ShuttleQuickMessages userType="guest" onSendQuickMessage={handleSendMessage} />

                {/* Message panel */}
                <ShuttleMessagePanel
                  messages={messages}
                  currentSenderType="guest"
                  currentSenderId={guestId}
                  currentSenderName={guestName}
        ...
        {isCompleted && (
          <div className="bg-white rounded-2xl p-6 shadow-lg text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">送迎が完了しました</h3>
            <p className="text-gray-600">
              ○○旅館でのご滞在をお楽しみください。
              <br />
              お帰りの送迎もご予約いただけます。
        ...
        {!isCompleted && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            ...
            <ul className="text-sm text-amber-700 space-y-1">
              <li>・ 到着されましたら「到着しました」ボタンをタップしてください</li>
