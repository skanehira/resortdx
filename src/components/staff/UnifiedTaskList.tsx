...
interface UnifiedTaskListProps {
	...
	onEquipmentReport?: (roomNumber: string, report: EquipmentReport) => void;
...
export const UnifiedTaskList = ({
	...
	const [internalCategoryFilter, setInternalCategoryFilter] =
		useState<CategoryFilter>(externalCategoryFilter ?? "all");
	...
	const [statusFilter, setStatusFilter] = useState<UnifiedTaskStatus | "all">(
		"all",
	);
		...
		return tasks.filter((task) => {
			...
			if (task.type === "help_request" && task.helpRequest) {
				...
				if (hr.helpStatus === "pending") {
					...
					if (
						Array.isArray(hr.targetStaffIds) &&
						hr.targetStaffIds.includes(currentStaff.id)
					)
						return true;
				}
				// 受諾済みで自分が受諾者の場合
	...
	const handleHousekeepingComplete = useCallback(
		(taskId: string, roomNumber: string) => {
			// 設備報告モーダルを表示
			setEquipmentReportModal({ isOpen: true, roomNumber, taskId });
		},
		[],
	);
	...
	const handleEquipmentReportSubmit = useCallback(
		(report: EquipmentReport) => {
			if (equipmentReportModal?.roomNumber) {
				onEquipmentReport?.(equipmentReportModal.roomNumber, report);
			}
			// タスクを完了にする
	...
	return (
		<div className="min-h-screen bg-[var(--shironeri)] pb-20">
			...
			{equipmentReportModal?.isOpen && equipmentReportModal.roomNumber && (
				<EquipmentReportModal
					roomNumber={equipmentReportModal.roomNumber}
					amenities={roomAmenities[equipmentReportModal.roomNumber] || []}
					equipment={roomEquipment[equipmentReportModal.roomNumber] || []}
