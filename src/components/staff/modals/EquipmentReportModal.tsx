        ...
        return (
          <button
            ...
            className={`px-2 py-1 rounded text-xs font-medium transition-colors ${
              isSelected ? "text-white" : "bg-[var(--nezumi)]/10 text-[var(--nezumi)]"
            }`}
...
export const EquipmentReportModal = ({
  ...
  const [amenityStates, setAmenityStates] = useState<Record<string, StockLevel>>(() =>
  ...
  >(() =>
    Object.fromEntries(equipment.map((e) => [e.id, { status: e.status, notes: e.notes || "" }])),
  );
