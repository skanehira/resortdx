		...
		<div className="shoji-panel p-4 border-l-3 border-l-[var(--ai)] animate-slide-up">
			...
			<div className="relative mb-4 px-2">
				<div className="flex items-center justify-between">
					{["not_departed", "heading", "arrived", "boarded", "completed"].map(
						(stage, index) => {
							const stages: ShuttleStatus[] = [
								"not_departed",
								"heading",
			...
			>
				<button onClick={onExpand} className="w-full text-left p-4">
					...
					<div className="mb-2">
						...
						{task.guestArrivalNotified && (
							<span className="ml-2 text-xs text-[var(--kincha)]">
								到着通知あり
							</span>
						)}
					</div>
...
export const MobileShuttleList = () => {
	...
	const currentVehicle = useMemo(() => {
		const currentTask = shuttleTasks.find(
			(t) =>
				t.shuttleStatus !== "completed" && t.shuttleStatus !== "not_departed",
		);
		return currentTask?.assignedVehicleId
			? getVehicleById(currentTask.assignedVehicleId)
			: null;
	}, [shuttleTasks]);
	...
	const sortedTasks = useMemo(() => {
		return [...shuttleTasks].sort((a, b) => {
			...
			const orderDiff =
				statusOrder(a.shuttleStatus) - statusOrder(b.shuttleStatus);
