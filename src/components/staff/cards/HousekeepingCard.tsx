          ...
          <div key={step.key} className="flex flex-col items-center">
            ...
            <span
              className={`text-xs mt-1 ${
                index + 1 <= currentStep ? "text-[var(--sumi)]" : "text-[var(--nezumi-light)]"
              }`}
            >
              {step.label}
            </span>
...
const InspectionFlow = ({ status }: InspectionFlowProps) => {
  const currentIndex = status === "pending" ? 0 : status === "in_progress" ? 1 : 2;
      ...
      {steps.map((step, index) => (
        <>
          <div key={step.key} className="flex flex-col items-center">
            ...
            <span
              className={`text-xs mt-1 ${
                index <= currentIndex ? "text-[var(--sumi)]" : "text-[var(--nezumi-light)]"
              }`}
            >
              {step.label}
            </span>
...
export const HousekeepingCard = ({
  ...
  const hasCleaning = housekeeping.category === "cleaning" && housekeeping.cleaningChecklist;
  const isTurndown = housekeeping.category === "turndown";
  const isBath = housekeeping.category === "bath";

  const allChecked = housekeeping.cleaningChecklist?.every((i) => i.isChecked) ?? false;

  const remainingCount = housekeeping.cleaningChecklist?.filter((i) => !i.isChecked).length ?? 0;

  const handleStatusChange = (taskId: string, newStatus: UnifiedTask["status"]) => {
    if (isInspection && newStatus === "completed" && task.roomId && onRequestEquipmentReport) {
      onRequestEquipmentReport(taskId, task.roomId);
      return;
    }
                ...
                <>
                  <div className="p-4 bg-[var(--aotake)]/10 border border-[var(--aotake)]/30 rounded-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-[var(--aotake)]">全項目完了!</span>
                    </div>
                  </div>
                  <button
                  ...
                  {task.roomId && onRequestEquipmentReport && (
                    <button
                      onClick={() => onRequestEquipmentReport(task.id, task.roomId!)}
        ...
        <div className="space-y-3">
              ...
              <div className="p-4 bg-[var(--kincha)]/10 border border-[var(--kincha)]/30 rounded-lg">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--nezumi)]">清掃品質と設備状態を確認します</p>
                  </div>
                </div>
            ...
            <>
              <div className="p-4 bg-[var(--kincha)]/10 border border-[var(--kincha)]/30 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-[var(--kincha)]">点検中</span>
                </div>
              ...
              <button
                onClick={() => task.roomId && onRequestEquipmentReport?.(task.id, task.roomId)}
          ...
          {task.status === "completed" && (
            <div className="p-4 bg-[var(--aotake)]/10 border border-[var(--aotake)]/30 rounded-lg">
              <div className="flex items-center gap-2">
                <CheckCircleIcon size={20} className="text-[var(--aotake)]" />
                <span className="font-medium text-[var(--aotake)]">点検完了</span>
              </div>
              {housekeeping.equipmentReport && (
                <p className="text-sm text-[var(--nezumi)] mt-2">
                  報告時刻:{" "}
                  {new Date(housekeeping.equipmentReport.inspectedAt).toLocaleTimeString("ja-JP")}
                </p>
              )}
            </div>
          )}
      ...
      {isTurndown && (
        <div className="space-y-3">
          <div className="p-3 bg-[var(--ai)]/10 rounded-lg">
            ...
            <p className="font-medium text-[var(--sumi)]">ターンダウンサービス</p>
            {housekeeping.description && (
              <p className="mt-1 text-sm text-[var(--nezumi)]">{housekeeping.description}</p>
            )}
          </div>
            ...
            <>
              <div className="p-4 bg-[var(--aotake)]/10 border border-[var(--aotake)]/30 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-[var(--aotake)]">準備中</span>
                </div>
              </div>
              <button
                onClick={() => onStatusChange(task.id, "completed")}
