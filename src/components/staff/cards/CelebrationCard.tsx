			...
			<div className="space-y-4">
					...
					<>
							...
							{celebration.items.map((itemCheck) => {
								...
								return (
									<button
										key={itemCheck.item}
										onClick={() =>
											onToggleCelebrationItem?.(task.id, itemCheck.item)
										}
									...
									>
										...
										{itemCheck.isChecked && (
											<CheckIcon size={16} className="text-[var(--shu)]" />
										)}
									</button>
								);
							})}
						...
						{allChecked && (
							<>
								...
								{/* Completion note */}
								<div className="space-y-2">
									<label className="block text-xs text-[var(--nezumi)]">
										完了メモ（任意）
									</label>
									<textarea
				...
				{task.status === "completed" && (
					<>
						...
						<div className="h-1.5 bg-[var(--shironeri-warm)] rounded-full overflow-hidden">
							<div
								className="h-full bg-[var(--aotake)] rounded-full"
								style={{ width: "100%" }}
							/>
						</div>
