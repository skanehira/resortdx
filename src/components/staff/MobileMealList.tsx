...
import { mockMealTasks, mockMealOrderNotifications, getStaffById } from "../../data/mockData";
import {
  DiningIcon,
  ClockIcon,
  CheckIcon,
...
const MealStatusBadge = ({ status, needsCheck }: { status: MealStatus; needsCheck?: boolean }) => {
  if (needsCheck) {
    return (
...
const MealProgressIndicator = ({ status }: { status: MealStatus }) => {
  ...
  const currentIndex = status === "needs_check" ? 0 : stages.indexOf(status as MealStatus);
    ...
    <div className="shoji-panel p-4 border-l-3 border-l-[var(--ai)] animate-slide-up">
      ...
      <div className="mb-3">
        <div className="text-lg font-display font-bold text-[var(--sumi)]">
          ...
          <span className="ml-2 text-sm font-normal text-[var(--nezumi)]">{task.guestCount}名</span>
        </div>
        <div className="flex items-center gap-2 text-sm text-[var(--nezumi)] mt-1">
          ...
          <span>
            {MEAL_TYPE_LABELS[task.mealType]} / {COURSE_TYPE_LABELS[task.courseType]}
          </span>
        </div>
      </div>
      ...
      {task.dietaryRestrictions.length > 0 && (
        <div className="mb-3 p-3 bg-[var(--shu)]/10 rounded-lg border border-[var(--shu)]/30">
          <div className="flex items-center gap-2 text-[var(--shu)] mb-1">
            <AllergyIcon size={16} />
            <span className="font-display font-semibold text-sm">アレルギー注意</span>
          </div>
          ...
          {task.dietaryNotes && (
            <p className="mt-2 text-xs text-[var(--sumi)]">{task.dietaryNotes}</p>
          )}
        </div>
      )}
      ...
      <div className="relative mb-4 px-2">
        <div className="flex items-center justify-between">
          {["preparing", "serving", "completed"].map((stage, index) => {
            const stages: MealStatus[] = ["preparing", "serving", "completed"];
            const currentIndex =
              task.mealStatus === "needs_check" ? 0 : stages.indexOf(task.mealStatus);
            const stageLabels = ["準備中", "配膳中", "完了"];
            ...
            return (
              <div key={stage} className="flex flex-col items-center z-10" style={{ flex: 1 }}>
                ...
                >
                  {index < currentIndex && <CheckIcon size={14} className="text-white" />}
                </div>
                <span
          ...
          style={{
            width: `${
              ((task.mealStatus === "needs_check"
                ? 0
                : ["preparing", "serving", "completed"].indexOf(task.mealStatus)) /
                2) *
              100
            }%`,
          }}
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-[var(--nezumi)] mt-1">{notification.createdAt}</p>
        </div>
        {!notification.isRead && (
          <button
            onClick={onMarkRead}
    ...
    <div className="relative overflow-hidden rounded-xl">
      ...
      {task.mealStatus !== "completed" && nextStatus && (
        <div
          ...
          style={{ width: "120px" }}
        >
          <span className="text-white font-medium">{buttonLabels[task.mealStatus]}</span>
        </div>
      )}
        ...
        <button onClick={onExpand} className="w-full text-left p-4">
          <div className="flex items-start justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <MealStatusBadge status={task.mealStatus} needsCheck={task.needsCheck} />
              {task.isAnniversaryRelated && (
          ...
          <div className="mb-2">
            ...
            <span className="ml-2 text-sm text-[var(--nezumi)]">{task.guestCount}名</span>
          </div>
            ...
            <div className="mt-4 pt-4 border-t border-[var(--shironeri-warm)] space-y-3 animate-slide-up">
              ...
              {task.dietaryRestrictions.length > 0 && (
                <div className="p-3 bg-[var(--shu)]/10 rounded-lg">
                  ...
                  {task.dietaryNotes && (
                    <p className="mt-1 text-xs text-[var(--sumi)]">{task.dietaryNotes}</p>
                  )}
                </div>
              )}
    ...
    <div className="min-h-screen bg-[var(--shironeri)] pb-20">
      ...
      <header className="sticky top-0 z-30 bg-[var(--shironeri)]/80 backdrop-blur-sm border-b border-[var(--shironeri-warm)]">
        <div className="px-4 py-3">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-lg font-display font-bold text-[var(--sumi)]">配膳タスク</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              {currentStaff && (
                <div className="flex items-center gap-2 px-3 py-1.5 bg-[var(--ai)]/10 rounded-full">
                  <span className="text-sm font-medium text-[var(--ai)]">{currentStaff.name}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <main className="p-4 space-y-4">
        ...
        {filteredTasks.length === 0 && (
          <div className="shoji-panel p-8 text-center">
            <DiningIcon size={48} className="mx-auto text-[var(--nezumi)]/30 mb-3" />
