		...
		<div className="space-y-2">
			{items.map((itemCheck) => (
				...
				>
					...
					{itemCheck.notes && (
						<span className="ml-auto text-xs text-[var(--nezumi)]">
							{itemCheck.notes}
						</span>
					)}
				</button>
	...
	return (
		<div className="shoji-panel p-4 border-l-3 border-l-[var(--kincha)] animate-slide-up">
			...
			<div className="mb-3">
				...
				<p className="mt-1 text-sm text-[var(--sumi)]">
...
export const MobileCelebrationList = () => {
	const [celebrationTasks, setCelebrationTasks] = useState<CelebrationTask[]>(
		() =>
			mockCelebrationTasks.filter(
				(t) => t.assignedStaffId === CURRENT_STAFF_ID,
			),
	);
	...
	const filterCounts = useMemo(() => {
		return {
			...
			in_progress: celebrationTasks.filter((t) => t.status === "in_progress")
				.length,
			completed: celebrationTasks.filter((t) => t.status === "completed")
				.length,
		};
	}, [celebrationTasks]);
			...
			prev.map((t) =>
				t.id === taskId
					? {
							...t,
							items: t.items.map((i) =>
								i.item === item ? { ...i, isChecked: !i.isChecked } : i,
							),
						}
					: t,
			),
				...
				{activeTask && activeFilter === "all" && (
					<CurrentCelebrationHighlight
						...
						onUpdateReport={(report) =>
							handleUpdateReport(activeTask.id, report)
						}
					/>
				)}
